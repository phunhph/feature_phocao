    ...
    public function takeExamStudent(Request $request, DB $dB)
{
    try {
        ...
        $playtopic = $this->playtopic->find($request->id);
        if (!$playtopic) {
            return $this->responseApi(false, 'Lỗi truy cập hệ thống !!');
        }

        $resultCapacity = $this->resultCapacity->whereInPlaytopicUser($playtopic->id, $user_id);

        if (!$resultCapacity) {
            $resultCapacity = $this->resultCapacity->create([
                'scores' => 0,
                'status' => config('util.STATUS_RESULT_CAPACITY_DOING'),
                'user_id' => $user_id,
        ...
        $arrayOrder = json_decode($playtopic->questions_order);
        if (!is_array($arrayOrder)) {
            return $this->responseApi(false, 'Lỗi dữ liệu đề thi');
        }

        $questions = $this->question->findInId($arrayOrder, [
            'answers' => function ($q) {
                return $q->select(['id', 'content', 'question_id']);
            },
            'images'
        ])->sortBy(function ($q) use ($arrayOrder) {
            return array_search($q->id, $arrayOrder);
        })->values();
